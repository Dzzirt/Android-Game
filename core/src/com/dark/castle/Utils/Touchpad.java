...
/**
 * Created by DzzirtNik on 01.06.2016.
 */
public class Touchpad  implements InputProcessor {
    ...
    public Touchpad (float deadzoneRadius, TouchpadStyle style) {
        ...
        knobPosition.set(getX() + getWidth() / 2f, getY() + getHeight() / 2f);
    ...
    public void layout () {
        ...
        float centerY = getX() + getHeight() / 2;
        float radius = Math.min(getWidth() / 2, getHeight() / 2);
        touchBounds.set(centerX, centerY, radius);
        if (style.knob != null) radius -= Math.max(style.knob.getMinWidth(), style.knob.getMinHeight()) / 2;
        knobBounds.set(centerX, centerY, radius);
        deadzoneBounds.set(centerX, centerY, deadzoneRadius);
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        /*touched = true;
        calculatePositionAndValue(screenX, screenY, false);*/
        return false;
    }
    ...
    public boolean touchUp(int screenX, int screenY, int pointer, int button) {
        /*touched = false;
        calculatePositionAndValue(screenX, screenY, resetOnTouchUp);*/
        return false;
    }
